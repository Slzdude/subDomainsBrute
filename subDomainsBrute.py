...
class SubNameBrute(object):
    def __init__(self, *params):
        ...
        self.resolvers = [
            dns.resolver.Resolver(configure=False) for _ in range(self.options.threads)
        ]
        ...
        self.outfile = open(
            "%s/%s_part_%s.txt" % (tmp_dir, self.domain, self.process_num), "w"
        )
    ...
    def scan(self, j):
        self.resolvers[j].nameservers = [
            self.dns_servers[j % self.dns_count]
        ] + self.dns_servers
        ...
        while True:
            ...
            try:
                ...
                if answers:
                    ...
                    try:
                        self.scan_count_local += 1
                        self.resolvers[j].query("test-not-existed." + cur_domain)
...
def wildcard_test(domain, level=1):
    try:
        ...
        answers = r.query("not-existed-test.%s" % domain)
        ips = ", ".join(sorted([answer.address for answer in answers]))
        if level == 1:
            print(f"any-sub.{domain.ljust(30)}\t{ips}")
...
if __name__ == "__main__":
    ...
    print
    "Output file is %s" % out_file_name
